...
@RestController
@CrossOrigin(origins = "*")
@RequestMapping("/rest/secure/goalProgress")
public class GoalProgressService {
	...
	public Collection<GoalItemProgress> getRange(@PathVariable Long startTS, @PathVariable Long endTS,
			HttpServletRequest request) {
		return jb.getRange(startTS, endTS);
	}

	@GetMapping("/{guid}")
	public GoalItemProgress get(@PathVariable String guid) {
		return jb.get(guid);
	}

	@PostMapping(consumes = "application/json", produces = "application/json")
	public CommonResponse.Status saveGoalProgress(@RequestBody GoalItemProgress journal) {
		return jb.update(journal);
	}

	@PostMapping(path = "/batch", consumes = "application/json", produces = "application/json")
	public Collection<CommonResponse.Status> saveBatchGoalProgress(
			@RequestBody Collection<GoalItemProgress> journalCollection, HttpServletRequest request) {
